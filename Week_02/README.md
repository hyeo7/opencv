...
# (최대 30번 반복하거나, 정확도가 0.001 이하로 떨어지면 탐색을 종료합니다.)
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001) 

# 실제 세계의 3D 좌표를 저장할 빈 배열을 생성합니다 (Z축은 0으로 가정하므로 평면 좌표계입니다).
objp = np.zeros((CHECKERBOARD[0]*CHECKERBOARD[1], 3), np.float32) 

# np.mgrid를 사용하여 체스보드의 x, y 격자 인덱스(0,1,2...)를 생성하고 2열 행렬로 형태를 바꿉니다.
objp[:, :2] = np.mgrid[0:CHECKERBOARD[0], 0:CHECKERBOARD[1]].T.reshape(-1, 2) 

# 생성된 인덱스에 격자 한 칸의 실제 크기(25.0)를 곱해 실제 스케일(mm)이 반영된 3D 좌표를 완성합니다.
objp *= square_size 
...
for fname in images: # glob으로 찾은 파일 경로 리스트를 하나씩 반복하며 처리합니다.
    ...
    if ret == True: # 만약 코너를 성공적으로 모두 찾았다면
        ...
        cv2.waitKey(50) # 화면에 띄운 뒤 50 밀리초(0.05초) 동안 잠시 대기합니다.
...
# 반환값: 결과 상태(ret), 내부 파라미터 행렬(K), 왜곡 계수(dist), 회전 벡터(rvecs), 이동 벡터(tvecs)
ret, K, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, img_size, None, None) 

print("Camera Matrix K:") # "Camera Matrix K:"라는 문구를 화면에 출력합니다.
print(K) # 계산된 카메라 내부 파라미터 행렬(초점 거리, 주점 등)을 출력합니다.

print("\nDistortion Coefficients:") # 한 줄 띄우고 "Distortion Coefficients:"라는 문구를 출력합니다.
print(dist) # 계산된 렌즈의 방사 왜곡 및 접선 왜곡 계수 배열을 출력합니다.
...
if len(images) > 0: # 처리한 이미지가 1장이라도 있다면 왜곡 보정 테스트를 진행합니다.
    ...
    # 원본 이미지에 계산된 내부 행렬(K)과 왜곡 계수(dist)를 적용하여 왜곡이 펴진 새 이미지를 만듭니다.
    undistorted_img = cv2.undistort(test_img, K, dist, None, K) 
    
    cv2.imshow('Original', test_img) # 왜곡 보정 전의 원본 이미지를 'Original' 창에 띄웁니다.
    cv2.imshow('Undistorted', undistorted_img) # 왜곡이 보정된 이미지를 'Undistorted' 창에 띄웁니다.
...
<img width="738" height="195" alt="image" src="https://github.com/user-attachments/assets/a87ca9b4-a311-46aa-bf35-4c11643c181e" />
</div>
<br><br>
...
rois = {
    "Painting": (55, 50, 130, 110), # 왼쪽 위 그림 액자 영역
    "Frog": (90, 265, 230, 95), # 아래쪽 초록색 개구리 인형 영역
    "Teddy": (310, 35, 115, 90) # 오른쪽 위 분홍색 곰 인형 영역
}
...
# 4. 결과 출력
# -----------------------------
print("=== ROI별 평균 Disparity 및 Depth ===") # 콘솔창에 결과 시작을 알리는 문구를 출력합니다.
...
for name, res in results.items(): # 계산된 영역별 결과 딕셔너리를 하나씩 순회합니다.
    # 각 영역의 이름과 평균 시차(소수점 2자리), 평균 깊이(소수점 4자리)를 콘솔에 출력합니다.
...
# 모든 비교가 끝난 후 최종적으로 가장 가까운 객체와 먼 객체를 분석하여 콘솔에 출력합니다.
...
d_min = np.nanpercentile(disp_tmp, 5) # 극단적인 노이즈를 피하기 위해 하위 5% 위치의 시차 값을 최솟값 기준으로 잡습니다.
d_max = np.nanpercentile(disp_tmp, 95) # 극단적인 노이즈를 피하기 위해 상위 95% 위치의 시차 값을 최댓값 기준으로 잡습니다.
...
# 9. 화면 출력
# -----------------------------
# 이전 지시사항 반영: 박스가 그려지지 않은 실제 원본 왼쪽 이미지를 'Original' 창에 띄웁니다.
